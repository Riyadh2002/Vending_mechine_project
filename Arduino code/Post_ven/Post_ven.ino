...
void handleBody(AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
    ...
    if (!error) {
        ...
        if (id == 1) {
            // Motor 1 rotation based on quantity, with reversed direction
            analogWrite(enablePin1, 255); // Increase to 100% duty cycle
    for (int q = 0; q < quantity; q++) {
        digitalWrite(motor1Pin1, LOW);  // Reverse direction
        digitalWrite(motor1Pin2, HIGH); // Reverse direction
        
        delay(700); // Adjust for one 360-degree rotation based on your motor's speed
    ...
    digitalWrite(enablePin1, LOW); // Turn off Motor 1
            ...
            // Motor 2 rotation based on quantity, with reversed direction
            analogWrite(enablePin1, 188); // Increase to 100% duty cycle
    analogWrite(enablePin2, 255); // Increase to 100% duty cycle
    for (int q = 0; q < quantity; q++) {
        digitalWrite(motor2Pin1, LOW);  // Reverse direction
        digitalWrite(motor2Pin2, HIGH); // Reverse direction
